    ...
    func addTrip() {
        Task {
            do {
                ...
                let startDate = DateFormatter.localizedString(from: Date(), dateStyle: .short, timeStyle: .short)
                let endDate = DateFormatter.localizedString(from: Calendar.current.date(byAdding: .hour, value: 2, to: Date())!, dateStyle: .short, timeStyle: .short)
